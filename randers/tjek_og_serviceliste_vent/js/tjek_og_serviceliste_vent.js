...
{
	var list = createList();
	jQuery('input#anlaeg').autocomplete({
		selectFirst : true,
		source: list,
		delay: 200,
		minLength : 3,
...
{
	var params = {
		page: 'formular.send.tjek_og_serviceliste_vent.readall'
	};
	...
	jQuery.ajax( {
		...
		success : function(data, status) 
		{
			...
			for (var i=0;i<rows.length;i++) 
			{
				...
				var curValue = jQuery(rows[i]).find('col[name="adresse"]').text() + ' (' +jQuery(rows[i]).find('col[name="anlaeg_nr"]').text() + ') "' +jQuery(rows[i]).find('col[name="stednavn"]').text() + '"'
...
function elementSelectedAnmeld(event, ui)
{
	...
	jQuery.ajax( {
		...
		success : function(data, status) 
		{
			...
			for (var i=0;i<rows.length;i++) 
			{
				curWKT = jQuery(rows[i]).find('col[name="shape_wkt"]').text();
			}
		}
	});
	jQuery('#anlaeg_wkt').val(curWKT);
