...
<!--                     <fo:region-after extent="5mm" border-top="0.1pt solid black"/> -->
    ...
    <xsl:template name="bodyTemplate">
        ...
        <fo:block linefeed-treatment="preserve">
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			<xsl:value-of select="//row/col[@name='konflikttekst']"/>
        </fo:block>
        <fo:block linefeed-treatment="preserve">
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			Adresse: <xsl:value-of select="//row/col[@name='adresse']"/>
        </fo:block>

		<fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			Bygherre: <xsl:value-of select="//row/col[@name='bygherre']"/>
        </fo:block>
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			Kloakmester, som har udført arbejdet: <xsl:value-of select="//row/col[@name='vvs_mester']"/>.
        </fo:block>
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			Kloakmesterens email: <xsl:value-of select="//row/col[@name='email']"/>.
        </fo:block>
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			Dato for afslutning af arbejdet: <xsl:value-of select="//row/col[@name='dato']"/>
        </fo:block>
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			Tagfladevand: <xsl:value-of select="//row/col[@name='tagfladevand']"/>
        </fo:block>
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
        ...
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			Vaskeplads: <xsl:value-of select="//row/col[@name='vaskeplads']"/>
        </fo:block>
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
			Tilsluttet areal fra vaskeplade &gt; 20m2: <xsl:value-of select="//row/col[@name='vaskeplads_str']"/>
        </fo:block>
		<xsl:variable name="moedding" select="'true'" >
		</xsl:variable>
		<xsl:choose>
		...
		<xsl:variable name="erhvervsareal" select="'0'" ></xsl:variable>
        ...
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
            Kommentar: <xsl:value-of select="//row/col[@name='kommentar']"/>
        </fo:block>
        ...
        <fo:block>
            <xsl:attribute name="margin-top">3mm</xsl:attribute>
            Vedhæftet fil: <xsl:value-of select="//row/col[@name='skitse']"/>
