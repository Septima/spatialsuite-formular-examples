...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="konflikttekst2">Ingen ejendomme fundet</xsl:param>
  <xsl:param name="art">Ikke oplyst</xsl:param>
  <xsl:param name="sted">Ikke oplyst</xsl:param>
  ...
  <xsl:param name="arttekst">Ikke oplyst</xsl:param>
  <xsl:param name="stedtekst">Ikke oplyst</xsl:param>
  <xsl:param name="filnavn">Ingen filer vedhæftet!</xsl:param>
  <xsl:param name="filnavn2">Ingen filer vedhæftet!</xsl:param>
  <xsl:param name="navn">Ikke oplyst</xsl:param>
  <xsl:param name="adresseansvarlig">Ikke oplyst</xsl:param>
  ...
  <xsl:attribute-set name="footer-cell" >
	...
	<fo:block font-size="14pt" >
		<xsl:attribute name="margin-top">3mm</xsl:attribute>
		<fo:inline font-weight="bold">Ansøgning om afvikling af arrangementer for mere end 150 personer.</fo:inline>
	</fo:block>
	...
	<fo:block>
		...
		Det ansøgte areal berører: <xsl:value-of select="$konflikttekst1"/> 
	</fo:block>  
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		Det ansøgte areal berører: <xsl:value-of select="$konflikttekst2"/> 
	</fo:block>   
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		Arrangementets art: <xsl:value-of select="$art"/> 
	</fo:block>   
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		Arrangementets sted: <xsl:value-of select="$sted"/> 
	</fo:block>   
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
	...
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		Arrangementet starter kl.: <xsl:value-of select="$start_tid"/> 
	</fo:block>  
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		Arrangementet slutter kl.: <xsl:value-of select="$slut_tid"/> 
	</fo:block>  
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
	...
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		Adresseansvarlig: <xsl:value-of select="$adresseansvarlig"/> 
	</fo:block>  
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
	...
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		Vedhæftet fil med Brand- og Evakueringsinstruks: <xsl:value-of select="$filnavn2"/> 
	</fo:block>  
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		Vedhæftet fil med pladsfordelingsplan: <xsl:value-of select="$filnavn"/> 
	</fo:block>  
