...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="uffe">[module:formular-randers.dir]/images/uffe_underskrift.jpg</xsl:param>
  ...
  <xsl:attribute-set name="footer-cell" >
            ...
            <fo:table-row>
				<fo:table-cell> 
					<fo:block>
						...
						<xsl:value-of select="$firmaadresse"/>
					</fo:block>
				</fo:table-cell>
				<fo:table-cell> 
					<fo:block >
	...
	<fo:block font-size="14pt" >
		<xsl:attribute name="margin-top">3mm</xsl:attribute>
		<fo:inline font-weight="bold">Afslag på mulighed for udstykning</fo:inline>
	</fo:block>
	<!-- HEREFTER KOMMER DE VANLIGE INFORMATIONER FRA FORMULAREN - FOERSTE SAET DATA HAR EN 'MARGIN-TOP' PAA 3MM - RESTEN 2MM -->
	<fo:block>
		<xsl:attribute name="margin-top">2mm</xsl:attribute>
		De har bedt om udtalelse om, hvorvidt der fra ejendommen <xsl:value-of select="$konflikttekst1" /> (<xsl:value-of select="$samletmatr" />) kan udstykkes en grund til selvstændig bebyggelse.
	</fo:block>				


	<xsl:choose>
		<xsl:when test="$landzone = 'ja'">
			<fo:block>
				<xsl:attribute name="margin-top">6mm</xsl:attribute>
				Ejendommen er beliggende i landzone og har et registreret areal på: <xsl:value-of select="$samletareal" /> m2
			</fo:block>				
		</xsl:when>
		<xsl:when test="$kommuneplan = 'ja'">
			<fo:block>
				<xsl:attribute name="margin-top">6mm</xsl:attribute>
				Ejendommen er underlagt en kommuneplan og har et registreret areal på: <xsl:value-of select="$samletareal" /> m2
			</fo:block>				
		</xsl:when>
		<xsl:when test="$lokalplan = 'ja'">
			<fo:block>
				<xsl:attribute name="margin-top">6mm</xsl:attribute>
				Ejendommen er underlagt en lokalplan og har et registreret areal på: <xsl:value-of select="$samletareal" /> m2
			</fo:block>				
		</xsl:when>
		<xsl:otherwise>
			<fo:block>
		...
		<fo:block>
			<xsl:attribute name="margin-top">6mm</xsl:attribute>
			Ejendommen er beliggende udenfor de områder, hvor der i flg. kommuneplanen for Randers Kommune kan ske byudvikling, og da ejendommen endvidere er beliggende i landzone, kan vi meddele Dem, at der ikke kan ske udstykning af en grund til selvstændigbebyggelse fra ejendommen.
		</fo:block>				

		<fo:block>
			<xsl:attribute name="margin-top">2mm</xsl:attribute>
			Henvendelse om dette brev kan ske til undertegnede på tlf. 8915 1552 eller på e-mail:ujrc@randers.dk
		</fo:block>				
		...
		<fo:block>  
			<xsl:attribute name="margin-top">10mm</xsl:attribute>
		   <fo:external-graphic height="18mm" width="44mm" 
			   content-height="25mm" content-width="44mm">
			   <xsl:attribute name="src">url('<xsl:value-of 
				   select="$uffe"/>')</xsl:attribute>
		   </fo:external-graphic>
		</fo:block> 
